...
for index, student in enumerate(classroom._students):
    ...
    x_2 = np.array([i[2] for i in student.progression])
    if len(student.no_progression) > 0:
        plt.plot(x_1 + 1, [i[1] for i in student.no_progression], 'ro', markersize=15, label="Presença para todos")
    plt.plot(x_2 + 1, [i[1] for i in student.progression], "bo", markersize=10, label="Pontuação individual")

    x_3 = list(x_1) + list(x_2)
    x_3.sort()
    plt.gca().set_xticks(np.array(x_3) + 1)
